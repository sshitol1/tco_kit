...
class MyExtension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id):
        ...
        self.liquid_cooling_type_1 = None
        self.liquid_cooling_type_secondary_1 = None
        self.liquid_cooling_type_2 = None
        self.liquid_cooling_type_secondary_2 = None
        ...
        self._capex_comparison_chart_window = ui.Window("CAPEX Comparision Chart", width=750, height=800, visible = False)
        self.capex_comparision_chart_container = self._capex_comparison_chart_window.frame

        self._opex_comparison_chart_window = ui.Window("OPEX Comparision Chart", width=750, height=800, visible = False)
        self.opex_comparision_chart_container = self._opex_comparison_chart_window.frame
                    ...
                    try:
                        ...
                        with ui.CollapsableFrame(" Facility Specification", style=self.STYLES["section_frame"]):
                            with ui.VStack(style=self.STYLES["input_container"]):
                                ...
                                with ui.HStack(height=30) as self.fws_air_container_1:
                        ...
                        with ui.CollapsableFrame(" Facility Specification 1", style=self.STYLES["section_frame"]):
                            with ui.VStack(style=self.STYLES["input_container"]):
                                ...
                                # Container for fws_liquid_menu ComboBox
                                with ui.HStack(height=30) as self.fws_liquid_container_1:
                                    ui.Label("FWS Design Temp (Liquid):", width=150, style=self.STYLES["label"])
                        ...
                        with ui.CollapsableFrame(" Facility Specification 2", style=self.STYLES["section_frame"]):
                            with ui.VStack(style=self.STYLES["input_container"]):
                                ...
                                # Container for fws_air_menu ComboBox
                                with ui.HStack(height=30) as self.fws_air_container_2:
                                    ui.Label("FWS Design Temp (Air):", width=210, style=self.STYLES["label"])
                                    self.fws_air_menu_2 = ui.ComboBox(0, *self.fws_air_options, style = self.STYLES["combo_box"])  # Initial values
                                    ui.Label("°C", width=30)

                                # Container for fws_liquid_menu ComboBox
                                with ui.HStack(height=30) as self.fws_liquid_container_2:
                                    ui.Label("FWS Design Temp (Liquid):", width=150, style=self.STYLES["label"])
                                ...
                                with ui.HStack(height=30):
                                    ui.Button("Show OPEX Comparison", clicked_fn=self.show_opex_comparison)
                        ...
                        self.num_pods_field.model.add_value_changed_fn(lambda model: self.update_calculations_1())
                        self.num_pods_field.model.add_value_changed_fn(lambda model: self.update_calculations_2())
                        ...
                        self.num_years_field.model.add_value_changed_fn(lambda model: self.update_calculations_1())
                        self.num_years_field.model.add_value_changed_fn(lambda model: self.update_calculations_2())
                        ...
                        self.pod_menu.model.get_item_value_model().add_value_changed_fn(lambda model: self.update_calculations_1())
                        self.pod_menu.model.get_item_value_model().add_value_changed_fn(lambda model: self.update_calculations_2())
                        ...
                        self.fws_liquid_menu.model.get_item_value_model().add_value_changed_fn(lambda model: self.update_calculations())
                        self.fws_liquid_menu_1.model.get_item_value_model().add_value_changed_fn(lambda model: self.update_calculations_1())
                        self.fws_liquid_menu_2.model.get_item_value_model().add_value_changed_fn(lambda model: self.update_calculations_2())
                        # self.fws_air_menu.model.get_item_value_model().add_value_changed_fn(lambda model: self.update_calculations())
                        self.fws_liquid_menu.model.get_item_value_model().add_value_changed_fn(lambda model: self.calculate_monthly_power_consumption())
                        ...
                        self.chiller_menu.model.get_item_value_model().add_value_changed_fn(
                            lambda model: self.update_calculations())
                        self.chiller_menu_1.model.get_item_value_model().add_value_changed_fn(
                            lambda model: self.update_calculations_1())
                        self.chiller_menu_2.model.get_item_value_model().add_value_changed_fn(
                            lambda model: self.update_calculations_2())
                        self.chiller_menu.model.get_item_value_model().add_value_changed_fn(
                            lambda model: self.calculate_monthly_power_consumption())
                        
                        self.dryer_menu.model.get_item_value_model().add_value_changed_fn(
                            lambda model: self.update_calculations())
                        self.dryer_menu_1.model.get_item_value_model().add_value_changed_fn(
                            lambda model: self.update_calculations_1())
                        self.dryer_menu_2.model.get_item_value_model().add_value_changed_fn(
                            lambda model: self.update_calculations_2())
                        self.dryer_menu.model.get_item_value_model().add_value_changed_fn(
                            lambda model: self.calculate_monthly_power_consumption())
                        # Initial update
    ...
    def update_fws_design_temperature_liquid(self, tcs_liquid_value):
        ...
        self.fws_liquid_container_1.clear()
        with self.fws_liquid_container_1:
            ui.Label("FWS Design Temp (Liquid):", width=150, style=self.STYLES["label"])
            self.fws_liquid_menu_1 = ui.ComboBox(0, *[str(temp) for temp in liquid_temp_range], style = self.STYLES["combo_box"])  # Populate with calculated range
            ui.Label("°C", width=30)

        self.fws_liquid_container_2.clear()
        with self.fws_liquid_container_2:
            ui.Label("FWS Design Temp (Liquid):", width=150, style=self.STYLES["label"])
            self.fws_liquid_menu_2 = ui.ComboBox(0, *[str(temp) for temp in liquid_temp_range], style = self.STYLES["combo_box"])  # Populate with calculated range
            ui.Label("°C", width=30)
        ...
        self.fws_liquid_menu_1.model.get_item_value_model().add_value_changed_fn(
                lambda model: self.update_liquid_cooling_options_1(int(self.fws_liquid_options[model.as_int]))
                        )
        self.fws_liquid_menu_2.model.get_item_value_model().add_value_changed_fn(
                lambda model: self.update_liquid_cooling_options_2(int(self.fws_liquid_options[model.as_int]))
                        )
        self.fws_liquid_menu.model.get_item_value_model().add_value_changed_fn(
                lambda model: self.calculate_monthly_power_consumption()
                        )
    ...
    def update_fws_design_temperature_air(self,air_supply_temp):
        try:
            ...
            self.fws_air_container_1.clear()
            with self.fws_air_container_1:
                ui.Label("FWS Design Temp (Air):", width=150, style=self.STYLES["label"])
                # Populate ComboBox with the new air temperature range as strings
                self.fws_air_menu_1 = ui.ComboBox(0, *[str(temp) for temp in air_temp_range],style = self.STYLES["combo_box"])
                ui.Label("°C", width=30)

            self.fws_air_container_2.clear()
            with self.fws_air_container_2:
                ui.Label("FWS Design Temp (Air):", width=150, style=self.STYLES["label"])
                # Populate ComboBox with the new air temperature range as strings
                self.fws_air_menu_2 = ui.ComboBox(0, *[str(temp) for temp in air_temp_range],style = self.STYLES["combo_box"])
                ui.Label("°C", width=30)
            ...
            self.fws_air_menu.model.get_item_value_model().add_value_changed_fn(lambda model: self.update_calculations())
            self.fws_air_menu_1.model.get_item_value_model().add_value_changed_fn(lambda model: self.update_calculations_1())
            self.fws_air_menu_2.model.get_item_value_model().add_value_changed_fn(lambda model: self.update_calculations_2())
    ...
    def plot_capex(self, total_common_capex, total_ac_air_cooled_chiller_capex,total_ac_water_cooled_chiller_capex, total_lc_air_cooled_chiller_capex,
        ...
        total_cost_per_power = total_capex / total_power
        ...
        with self.legend_container:
            with ui.VStack(width=ui.Fraction(1), height=ui.Fraction(1), spacing=5): # Use HStack to divide horizontally
                ...
                with ui.VStack(height=ui.Fraction(0.7), style=self.STYLES["main_container"], spacing=5):
                    ...
                    create_hover_label(f"Dollar per KW: {total_cost_per_power:.0f}")
    ...
    def plot_opex(self, total_common_opex_nth_year, total_ac_chiller_opex_nth_year,
        ...
        total_opex = (total_common_opex_nth_year + total_ac_chiller_opex_nth_year + total_lc_opex_nth_year)
        total_cost_per_power = total_opex / total_power
    ...
    def plot_capex_comparison(self):
        """
        Render a clustered bar chart for CAPEX comparison across scenarios.
        Each CAPEX category will have a group of vertically stacked bars for each scenario.
        """
        ...
        selected_chiller = self.chiller_options[self.chiller_menu.model.get_item_value_model().as_int]
        selected_dryer = self.dryer_options[self.dryer_menu.model.get_item_value_model().as_int]
        liquid_cooling_facility_1 = (
            selected_chiller if self.liquid_cooling_type == "Chiller"
            else selected_dryer
            )

        selected_chiller_1 = self.chiller_options[self.chiller_menu_1.model.get_item_value_model().as_int]
        selected_dryer_1 = self.dryer_options[self.dryer_menu_1.model.get_item_value_model().as_int]
        liquid_cooling_facility_2 = (
            selected_chiller_1 if self.liquid_cooling_type_1 == "Chiller"
            else selected_dryer_1
            )

        selected_chiller_2 = self.chiller_options[self.chiller_menu_2.model.get_item_value_model().as_int]
        selected_dryer_2 = self.dryer_options[self.dryer_menu_2.model.get_item_value_model().as_int]
        liquid_cooling_facility_3 = (
            selected_chiller_2 if self.liquid_cooling_type_2 == "Chiller"
            else selected_dryer_2
            )
        AC_Capex_1 = self.capex_data.get("AC Air Cooled")
        AC_Capex_2 = self.capex_data_1.get("AC Air Cooled")
        AC_Capex_3 = self.capex_data_2.get("AC Air Cooled")
        LC_Capex_1 = self.capex_data.get("LC Air Cooled")
        LC_Capex_2 = self.capex_data_1.get("LC Air Cooled")
        LC_Capex_3 = self.capex_data_2.get("LC Air Cooled")

        normal_style = {"font_size": 20, "color": cl("#FFFFFF")}  # Normal style
        hover_style = {"font_size": 22, "color": cl("#FFFFFF")}
        with self._capex_comparison_chart_window.frame:
            with ui.HStack():
                # Left side - Chart container
                with ui.VStack(width=ui.Fraction(0.6)):  # Takes 70% of the width
                    self.capex_comparision_chart_container = ui.VStack()
               
                # Right side - Legend container
                with ui.VStack(width=ui.Fraction(0.4)):  # Takes 30% of the width
                    self.legend_container = ui.VStack()

        total_capex_scenario_1 = sum(self.capex_data.values())
        total_cost_per_power_1 = total_capex_scenario_1 / total_power
        total_capex_scenario_2 = sum(self.capex_data_1.values())
        total_cost_per_power_2 = total_capex_scenario_2 / total_power
        total_capex_scenario_3 = sum(self.capex_data_2.values())
        total_cost_per_power_3 = total_capex_scenario_3 / total_power
        ...
        print("Capex data, capex_1, capex_2:", self.capex_data, self.capex_data_1, self.capex_data_2)
        print("Combined data:", combined_capex_data)

        # Calculate the maximum value for scaling bar heights
        max_value = max(max(values) for values in combined_capex_data.values()) if combined_capex_data else 1

        # Define colors for scenarios
        scenario_colors = [cl("#E6D9A2"), cl("#624E88"), cl("#CB80AB")]   
        scenario_names = ["Design 1", "Design 2", "Design 3"]

        # Ensure the CAPEX chart window is visible
        self._capex_comparison_chart_window.visible = True

        # Clear the chart container
        self.capex_comparision_chart_container.clear()
        ...
        # Render the chart
        with self.capex_comparision_chart_container:
            with ui.VStack(style=self.STYLES["main_container"], spacing=20):
                # Chart Title
                ui.Label(f"{main_city_name}'s CAPEX Comparison Across Scenarios", style=self.STYLES["title"], alignment=ui.Alignment.CENTER)

                # Categories and their bars
                with ui.HStack(spacing=30):  # Wider spacing for groups
                    for category, values in combined_capex_data.items():

                        # Render the bars for this category (one per scenario)
                        with ui.HStack(spacing=5):
                            for value, color in zip(values, scenario_colors):
                                with ui.VStack(alignment=ui.Alignment.CENTER):
                                    formatted_value = helper.format_large_number(value)
                                    ui.Label(f"{formatted_value}", style=self.STYLES["highlight_label"], alignment=ui.Alignment.CENTER)
                                    bar_height = (value / max_value) * 300  # Scale to a max height of 300
                                    ui.Rectangle(width=20, height=bar_height, style={"background_color": color})
                            
                with ui.HStack(spacing = 30, alignment=ui.Alignment.LEFT):
                    for category, values in combined_capex_data.items():
                        ui.Label(category, style=self.STYLES["highlight_label"], alignment=ui.Alignment.CENTER)

                    # Render the legend and summary in the right container
        with self.legend_container:
            with ui.VStack(width=ui.Fraction(1), height=ui.Fraction(1), spacing=5): # Use HStack to divide horizontally
                # Legend section with 30% width
                with ui.VStack(height=ui.Fraction(0.25), style=self.STYLES["main_container"], spacing=5):
                    ui.Label("Legend", style=self.STYLES["title"], alignment=ui.Alignment.CENTER)
                ...
                # Summary section with 70% width
                with ui.VStack(height=ui.Fraction(0.25), style=self.STYLES["main_container"], spacing=5):
                    ui.Label("Design 1", style=self.STYLES["title"], alignment=ui.Alignment.CENTER)
                    # create_hover_label(f"Annual PUE : {total_pue:.2f}")
                    create_hover_label(f"Liquid Cooling Facility : {self.liquid_cooling_type}")
                    # create_hover_label(f"POD type: {selected_pod_type}")
                    create_hover_label(f"Air-Cooled Chiller CAPEX: {helper.format_large_number(AC_Capex_1)}")
                    create_hover_label(f"Liquid Cooling {self.liquid_cooling_type} CAPEX: {helper.format_large_number(LC_Capex_1)}")
                    create_hover_label(f"Total CAPEX: ${helper.format_large_number(total_capex_scenario_1)}")
                    create_hover_label(f"Dollar per KW : {total_cost_per_power_1:.0f}")
                with ui.VStack(height=ui.Fraction(0.25), style=self.STYLES["main_container"], spacing=5):
                    ui.Label("Design 2", style=self.STYLES["title"], alignment=ui.Alignment.CENTER)
                    # create_hover_label(f"Annual PUE : {total_pue:.2f}")
                    create_hover_label(f"Liquid Cooling Facility : {self.liquid_cooling_type_1}")
                    # create_hover_label(f"POD type: {selected_pod_type}")
                    create_hover_label(f"Air-Cooled Chiller CAPEX: {helper.format_large_number(AC_Capex_2)}")
                    create_hover_label(f"Liquid Cooling {self.liquid_cooling_type_1} CAPEX: {helper.format_large_number(LC_Capex_2)}")
                    create_hover_label(f"Total CAPEX: ${helper.format_large_number(total_capex_scenario_2)}")
                    create_hover_label(f"Dollar per KW : {total_cost_per_power_2:.0f}")

                with ui.VStack(height=ui.Fraction(0.25), style=self.STYLES["main_container"], spacing=5):
                    ui.Label("Design 3", style=self.STYLES["title"], alignment=ui.Alignment.CENTER)
                    # create_hover_label(f"Annual PUE : {total_pue:.2f}")
                    create_hover_label(f"Liquid Cooling Facility : {self.liquid_cooling_type_2}")
                    # create_hover_label(f"POD type: {selected_pod_type}")
                    create_hover_label(f"Air-Cooled Chiller CAPEX: {helper.format_large_number(AC_Capex_3)}")
                    create_hover_label(f"Liquid Cooling {self.liquid_cooling_type_2} CAPEX: {helper.format_large_number(LC_Capex_3)}")
                    create_hover_label(f"Total CAPEX: ${helper.format_large_number(total_capex_scenario_3)}")
                    create_hover_label(f"Dollar per KW : {total_cost_per_power_3:.0f}")
    ...
    def show_capex_comparison(self):
        ...
        try:
            ...
            self._capex_chart_window.visible = False
            self._opex_comparison_chart_window.visible = False

            # Render the CAPEX comparison chart
            self.plot_capex_comparison()

            # Ensure the opex chart window is visible
            self._opex_comparison_chart_window.visible = True
            print("opex comparison chart displayed.")
        except Exception as e:
            print(f"Error displaying opex comparison chart: {e}")

    def plot_opex_comparison(self):
        """
        Render a clustered bar chart for opex comparison across scenarios.
        Each opex category will have a group of vertically stacked bars for each scenario.
        """
        # Combine opex data from all scenarios
        city_name_map = constants.city_name_map
        selected_city_full = self.unique_cities[self.city_menu.model.get_item_value_model().as_int]
        main_city_name = city_name_map.get(selected_city_full, selected_city_full)

        total_power = self.calculate_total_power()
        AC_Opex_1 = self.opex_data.get("AC Chiller")
        AC_Opex_2 = self.opex_data_1.get("AC Chiller")
        AC_Opex_3 = self.opex_data_2.get("AC Chiller")
        LC_Opex_1 = self.opex_data.get("LC")
        LC_Opex_2 = self.opex_data_1.get("LC")
        LC_Opex_3 = self.opex_data_2.get("LC")

        def calculate_bar_height(value, category):
            # Define scaling factors for different categories
            base_height = 300  # Base height for the tallest bar
            if category.startswith("Common"):
                scale_factor = 1.0  # No scaling for common categories
            elif "AC Chiller" in category:
                scale_factor = 10.0  # Increased scale for cooling categories
            elif "LC" in category:
                scale_factor = 10.0  # Decreased scale for air-related categories
            else:
                scale_factor = 1.0  # Default scaling for other categories
            
            # Calculate and return the scaled height
            return (value / max_value) * base_height * scale_factor

        normal_style = {"font_size": 20, "color": cl("#FFFFFF")}  # Normal style
        hover_style = {"font_size": 22, "color": cl("#FFFFFF")}
        with self._opex_comparison_chart_window.frame:
            with ui.HStack():
                # Left side - Chart container
                with ui.VStack(width=ui.Fraction(0.6)):  # Takes 70% of the width
                    self.opex_comparision_chart_container = ui.VStack()
               
                # Right side - Legend container
                with ui.VStack(width=ui.Fraction(0.4)):  # Takes 30% of the width
                    self.legend_container = ui.VStack()

        total_opex_scenario_1 = sum(self.opex_data.values())
        total_cost_per_power_1 = total_opex_scenario_1 / total_power
        total_opex_scenario_2 = sum(self.opex_data_1.values())
        total_cost_per_power_2 = total_opex_scenario_2 / total_power
        total_opex_scenario_3 = sum(self.opex_data_2.values())
        total_cost_per_power_3 = total_opex_scenario_3 / total_power

        combined_opex_data = {}
        for category in self.opex_data:
            combined_opex_data[category] = [
                self.opex_data.get(category, 0),    # Scenario 1
                self.opex_data_1.get(category, 0),  # Scenario 2
                self.opex_data_2.get(category, 0)   # Scenario 3
            ]
        print("Opex data, opex_1, opex_2:", self.opex_data, self.opex_data_1, self.opex_data_2)
        print("Combined data:", combined_opex_data)

        # Calculate the maximum value for scaling bar heights
        max_value = max(max(values) for values in combined_opex_data.values()) if combined_opex_data else 1
        ...
        # Clear the chart container
        self.opex_comparision_chart_container.clear()
        ...
        # Render the chart
        with self.opex_comparision_chart_container:
            with ui.VStack(style=self.STYLES["main_container"], spacing=20):
                # Chart Title
                ui.Label(f"{main_city_name}'s OPEX Comparison Across Scenarios", style=self.STYLES["title"], alignment=ui.Alignment.CENTER)

                # Categories and their bars
                with ui.HStack(spacing=30):  # Wider spacing for groups
                    for category, values in combined_opex_data.items():
                        ...
                        with ui.HStack(spacing=5):
                            for value, color in zip(values, scenario_colors):
                                with ui.VStack(alignment=ui.Alignment.CENTER):
                                    formatted_value = helper.format_large_number(value)
                                    ui.Label(f"{formatted_value}", style=self.STYLES["highlight_label"], alignment=ui.Alignment.CENTER)
                                    bar_height = calculate_bar_height(value, category) # Scale to a max height of 300
                                    ui.Rectangle(width=20, height=bar_height, style={"background_color": color})
        ...
        with self.legend_container:
            with ui.VStack(width=ui.Fraction(1), height=ui.Fraction(1), spacing=5): # Use HStack to divide horizontally
                ...
                with ui.VStack(height=ui.Fraction(0.25), style=self.STYLES["main_container"], spacing=5):
                    ...
                    create_hover_label(f"Liquid Cooling Facility : {self.liquid_cooling_type}")
                    # create_hover_label(f"POD type: {selected_pod_type}")
                    create_hover_label(f"Air-Cooled Chiller OPEX: {helper.format_large_number(AC_Opex_1)}")
                    create_hover_label(f"Liquid Cooling {self.liquid_cooling_type} OPEX: {helper.format_large_number(LC_Opex_1)}")
                    create_hover_label(f"Total OPEX: ${helper.format_large_number(total_opex_scenario_1)}")
                    create_hover_label(f"Dollar per KW : {total_cost_per_power_1:.0f}")
                ...
                with ui.VStack(height=ui.Fraction(0.25), style=self.STYLES["main_container"], spacing=5):
                    ...
                    create_hover_label(f"Liquid Cooling Facility : {self.liquid_cooling_type_1}")
                    # create_hover_label(f"POD type: {selected_pod_type}")
                    create_hover_label(f"Air-Cooled Chiller OPEX: {helper.format_large_number(AC_Opex_2)}")
                    create_hover_label(f"Liquid Cooling {self.liquid_cooling_type_1} OPEX: {helper.format_large_number(LC_Opex_2)}")
                    create_hover_label(f"Total OPEX: ${helper.format_large_number(total_opex_scenario_2)}")
                    create_hover_label(f"Dollar per KW : {total_cost_per_power_2:.0f}")
                ...
                with ui.VStack(height=ui.Fraction(0.25), style=self.STYLES["main_container"], spacing=5):
                    ...
                    create_hover_label(f"Liquid Cooling Facility : {self.liquid_cooling_type_2}")
                    # create_hover_label(f"POD type: {selected_pod_type}")
                    create_hover_label(f"Air-Cooled Chiller OPEX: {helper.format_large_number(AC_Opex_3)}")
                    create_hover_label(f"Liquid Cooling {self.liquid_cooling_type_2} OPEX: {helper.format_large_number(LC_Opex_3)}")
                    create_hover_label(f"Total OPEX: ${helper.format_large_number(total_opex_scenario_3)}")
                    create_hover_label(f"Dollar per KW : {total_cost_per_power_3:.0f}")

    def show_opex_comparison(self):
        """
        Handle the button click to display the OPEX comparison chart.
        """
        try:
            # Hide other charts (optional)
            self._pue_chart_window.visible = False
            self._opex_chart_window.visible = False
            self._capex_chart_window.visible = False
            self._capex_comparison_chart_window.visible = False

            # Render the OPEX comparison chart
            self.plot_opex_comparison()
