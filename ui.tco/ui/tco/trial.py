...
class MyExtension(omni.ext.IExt):
    ...
    def get_number_of_equipments_air_cooling(self,air_cooling_capacity_per_pod, num_pods, equipment_capacity_chillers):
        no_of_equipment_air = math.ceil((air_cooling_capacity_per_pod * num_pods)/ equipment_capacity_chillers)

        return no_of_equipment_air

    def get_number_of_equipments_liquid_cooling(self,liquid_cooling_capacity_per_pod, num_pods, equipment_capacity_chillers):
        no_of_equipment_liquid = math.ceil((liquid_cooling_capacity_per_pod * num_pods)/ equipment_capacity_chillers)
        return no_of_equipment_liquid


    def get_number_of_equipments_liquid_cooling_dry_cooler(self, fws_liquid, liquid_cooling_capacity_per_pod, num_pods):
        # Calculate the total cooling capacity required for all pods
        ...
        # Filter `dryer.csv` data based on `fws_liquid`
        matching_cooler_data = self.dryer_data[self.dryer_data['TWOUT'] == fws_liquid]
        matching_cooler_data['TA'] = matching_cooler_data['TA'].astype(int)
        print("Matching cooler data", matching_cooler_data['TA'])
        print(dry_bulb)
        ...
        print(possible_ta_values)

        if not possible_ta_values.empty:
            nearest_ta_above = possible_ta_values.min()
            cooling_capacity_per_unit = matching_cooler_data[matching_cooler_data['TA'] == nearest_ta_above]['Cooling Capacity'].iloc[0]
        else:
    ...
    def update_calculations(self):
        ...
        try:
            ...
            no_of_equipments_air = self.get_number_of_equipments_air_cooling(air_cooling_capacity_per_pod,num_pods,equipment_capacity_chillers)
            no_of_equipments_liquid = self.get_number_of_equipments_liquid_cooling(liquid_cooling_capacity_per_pod,num_pods,equipment_capacity_chillers)
            no_of_equipments_liquid_dry_cooler , cooling_capacity_per_unit_dry_cooler = self.get_number_of_equipments_liquid_cooling_dry_cooler(fws_liquid_temp,liquid_cooling_capacity_per_pod,num_pods)
            liquid_cooling_capacity = cooling_capacities.calculate_total_liquid_cooling_capacity(selected_pod_type)
            # Calculate liquid flow rate per pod and update the UI label
            required_liquid_flow_rate_per_pod = self.calculate_liquid_flow_rate_per_pod()
            ...
            air_chiller_power = power_calculations.calculate_air_cooling_power(city_data , chillers_data ,fws_air_temp, no_of_equipments_air, air_cooling_capacity_per_pod, num_pods)
            liquid_chiller_power = power_calculations.calculate_liquid_cooling_power(city_data, chillers_data, dryer_data, fws_liquid_temp,no_of_equipments_liquid,no_of_equipments_liquid_dry_cooler,liquid_cooling_capacity_per_pod,num_pods,cooling_capacity_per_unit_dry_cooler, liquid_cooling_type = self.liquid_cooling_type)
            ...
            # capex Air cooling
            total_cost_chiller_air_cooling = capex_calculations.calculate_chiller_air_cooling_cost(no_of_chillers_air_cooling_redundant,fws_liquid_temp)
            total_cost_chiller_liquid_cooling = capex_calculations.calculate_chiller_liquid_cooling_cost(no_of_chillers_liquid_cooling_redundant, no_of_dry_cooler_redundant, liquid_cooling_type = self.liquid_cooling_type )
    ...
    def calculate_monthly_power_consumption(self):
        ...
        monthly_air_cooling_power = monthly_calculation.calculate_monthly_air_chiller_power(city_data, chillers_data, fws_air_temp, no_of_equipments_air,air_cooling_capacity_per_pod,num_pods)
        monthly_liquid_cooling_power = monthly_calculation.calculate_monthly_liquid_chiller_power(city_data, chillers_data, dryer_data, fws_liquid_temp, no_of_equipments_liquid,no_of_equipments_liquid_dry_cooler,liquid_cooling_capacity_per_pod, num_pods, cooling_capacity_per_unit_dry_cooler, liquid_cooling_type)
        monthly_pue_values = []
    ...
    # def plot_capex(self, total_common_capex,total_ac_air_cooled_chiller_capex,total_ac_water_cooled_chiller_capex,total_lc_air_cooled_chiller_capex,total_lc_water_cooled_chiller_capex):
    #     """Render a CAPEX stacked bar chart with hover labels, tiered bar heights, and colored segments."""
       
    #     # Ensure the CAPEX chart window is visible
    #     self._capex_chart_window.visible = True
    #     self.capex_chart_container.clear()

    #     # Define CAPEX categories, values, and colors
    #     capex_categories = [
    #         "Total Common CAPEX",
    #         "CAPEX Air Cooling",
    #         "CAPEX Liquid Cooling"
    #     ]
    #     capex_values = [
    #         total_common_capex,
    #         total_ac_air_cooled_chiller_capex,
    #         total_lc_air_cooled_chiller_capex
    #     ]
    #     abbreviations = ["Common", "AC", "LC"]
    #     capex_colors = [
    #         cl("#b2e061"),  # Color for "Common"
    #         cl("#bd7ebe"),  # Color for "Air Cooling"
    #         cl("#ffee65")   # Color for "Liquid Cooling"
    #     ]

    #     # Fixed bar and layout dimensions
    #     bar_width = 50
    #     base_height = 10
    #     max_bar_height = 400
    #     total_capex = sum(capex_values)
    #     max_value = total_capex if total_capex else 1

    #     def calculate_bar_height(value):
    #         """Calculate bar height proportionally to the max value, scaled to max_bar_height."""
    #         return (value / max_value) * max_bar_height

    #     # Render the CAPEX stacked bar chart
    #     with self.capex_chart_container:
    #         with ui.VStack(style=self.STYLES["main_container"], spacing=5):
    #             # Chart Title
    #             ui.Label("CAPEX Stacked Overview", style=self.STYLES["title"], alignment=ui.Alignment.CENTER)

    #             # Render the stacked bar
    #             with ui.HStack(spacing=10, alignment=ui.Alignment.CENTER):
    #                 # Single bar representing the total CAPEX, divided into segments
    #                 with ui.VStack(alignment=ui.Alignment.BOTTOM):  # Align segments at the bottom
    #                     for value, color in zip(capex_values, capex_colors):
    #                         segment_height = calculate_bar_height(value)
    #                         ui.Rectangle(width=bar_width, height=segment_height, style={"background_color": color})

    #             # Display the total value above the stacked bar
    #             ui.Label(f"Total: ${total_capex:,.2f}", style=self.STYLES["highlight_label"], alignment=ui.Alignment.CENTER)

    #             # Legend for stacked bar segments
    #             with ui.HStack(spacing=10, alignment=ui.Alignment.LEFT):
    #                 for category, abbr, color in zip(capex_categories, abbreviations, capex_colors):
    #                     with ui.HStack(spacing=5):
    #                         # Color swatch for the legend
    #                         ui.Rectangle(width=20, height=20, style={"background_color": color})
    #                         # Abbreviation and category name
    #                         ui.Label(f"{abbr}: {category}", style=self.STYLES["label"], alignment=ui.Alignment.LEFT)
    ...
    def save_report(self):
        try:
            ...
            # Data
            monthly_data = self.calculate_monthly_power_consumption()
            monthly_pue_values = [data["monthly_pue"] for data in monthly_data.values()]
            month_names = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]

            capex_values = self.capex_data
            opex_values = self.opex_data
            ...
            capex_colors = [
                palegreen,  # Cornflower blue
                lavender,   # Tomato red
                lightgoldenrodyellow, # Gold
                paleturquoise,
                beige
            ]
            ...
            # Page 1: Monthly PUE
            pdf.drawString(50, 800, "Monthly PUE Overview")
            pue_chart = self.generate_logarithmic_pue_chart(
                month_names,
                monthly_pue_values,
                "Monthly PUE Overview",
                "Logarithmic Scale with Real Values"
            )
            renderPDF.draw(pue_chart, pdf, 50, 550)

            pdf.showPage()  # Start a new page

            # Page 2: CAPEX
            pdf.drawString(50, 800, "CAPEX Analysis")
            capex_chart = self.generate_bar_chart(
                capex_values,
                "CAPEX Overview",
                "CAPEX Distribution",
                capex_colors
            )
            renderPDF.draw(capex_chart, pdf, 50, 600)

            pdf.showPage()  # Start a new page

            # Page 3: OPEX
            pdf.drawString(50, 800, "OPEX Analysis")
            opex_chart = self.generate_bar_chart(
                opex_values,
                "OPEX Overview",
                "OPEX Distribution",
                opex_colors
            )
            renderPDF.draw(opex_chart, pdf, 50, 600)
    ...
    def generate_bar_chart(self, data_dict, title, subtitle, bar_colors):
        if not data_dict or all(v == 0 for v in data_dict.values()):
            raise ValueError("Data dictionary is empty or contains only zero values.")
        ...
        bar_chart.valueAxis.valueMax = max_value * 1.1
        bar_chart.valueAxis.valueStep = max_value // 5 if max_value >= 5 else 1

        # Debugging information
        print("DEBUG: Created VerticalBarChart object.")
        print(f"Type of bar_chart: {type(bar_chart)}")
        print(f"Type of bar_chart.bars: {type(bar_chart.bars)}")
        print(f"Value of bar_chart.bars: {bar_chart.bars}")

        # Assign colors to each bar in the dataset
        if hasattr(bar_chart, "bars") and len(bar_chart.bars) > 0:
            for idx, bar in enumerate(bar_chart.bars):
                # Set the fill color for each bar using the bar_colors list
                bar.fillColor = bar_colors[idx % len(bar_colors)]
        else:
            raise TypeError("bar_chart.bars is empty or not properly initialized.")

        # Add value labels above each bar
        for idx, value in enumerate(values):
            label = f"{value:.2f}"
            x = bar_chart.x + idx * (bar_chart.width / len(values)) + (bar_chart.width / (2 * len(values)))
            y = bar_chart.y + (value / max_value) * bar_chart.height
            drawing.add(String(x, y + 10, label, fontSize=8, textAnchor="middle"))

        # Add the bar chart to the drawing
        drawing.add(bar_chart)

        # Add the title and subtitle
        drawing.add(String(200, 280, title, fontSize=14, textAnchor="middle"))
        drawing.add(String(200, 260, subtitle, fontSize=10, textAnchor="middle"))

        return drawing


    def generate_logarithmic_pue_chart(self, month_names, monthly_pue_values, title, subtitle):
        if not monthly_pue_values or all(v <= 0 for v in monthly_pue_values):
            raise ValueError("PUE values are empty or contain non-positive values.")

        # Preprocess values: Apply logarithmic scaling
        scaled_values = [math.log10(v) if v > 0 else 0 for v in monthly_pue_values]
        original_values = monthly_pue_values  # Keep the original values for labels
        ...
        # Set up the chart with scaled data
        bar_chart.data = [scaled_values]
        bar_chart.categoryAxis.categoryNames = month_names

        # Y-Axis scaling and formatting
        bar_chart.valueAxis.valueMin = min(scaled_values) * 0.9
        bar_chart.valueAxis.valueMax = max(scaled_values) * 1.1
        bar_chart.valueAxis.labelTextFormat = "%.2f"  # Logarithmic values for the axis

        # Styling for bars
        bar_chart.bars[0].fillColor = palegreen  # Cornflower blue

        # Add value labels with original values
        for idx, (scaled_value, original_value) in enumerate(zip(scaled_values, original_values)):
            label = f"{original_value:.2f}"  # Use original value for the label
            x = bar_chart.x + idx * (bar_chart.width / len(scaled_values)) + (bar_chart.width / (2 * len(scaled_values)))
            y = bar_chart.y + (scaled_value / max(scaled_values)) * bar_chart.height
            drawing.add(String(x, y + 10, label, fontSize=8, textAnchor="middle"))
